...
use Shopware\Core\Framework\DataAbstractionLayer\Indexing\TreeUpdaterBag;
...
class TreeUpdater
{
    ...
    public function singleUpdate(string $parentId, string $entity, Context $context): array
    {
        $definition = $this->registry->getByEntityName($entity);
    ...
    private function updateLevelRecursively(array $updateIds, EntityDefinition $definition, Context $context, TreeUpdaterBag $bag): void
    {
        ...
        foreach ($updateIds as $updateId) {
            ...
            if ($entity !== null) {
                $this->updateEntity($definition, $entity, $pathField, $levelField, $context, $bag);
            }
        }
    ...
    private function updateEntity(EntityDefinition $definition, array $entity, ?TreePathField $pathField, ?TreeLevelField $levelField, Context $context, TreeUpdaterBag $bag): void
    {
        ...
        if ($this->updateEntityStatement === null) {
            $sql = sprintf('UPDATE `%s` SET ', $definition->getEntityName());
