...
class SchemaBuilderExtension extends SchemaBuilder
{
    ...
    public static array $fieldMapping = [
        ...
        DoubleField::class => Types::FLOAT, // Benötigt ein Typ für Geo Koordinaten
        ...
        // Umsortiert: Diese Felder stehen immer zuletzt in der Tabelle
    ...
    private function addForeignKeys(Table $table, EntityDefinition $definition): void
    {
        ...
        /** @var CompiledFieldCollection $fkFields */
        ...
        foreach ($fields as $field) {
            $fkField = $fkFields->getByStorageName($field->getStorageName());
            ...
            if ($reference->isVersionAware()) {
                ...
                $referenceVersionFieldStorageName = str_replace("_id", "_version_id", $field->getStorageName());
                foreach ($referenceVersionFields as $referenceVersionField) {
                    if ($referenceVersionField->getStorageName() === $referenceVersionFieldStorageName) {
                        $versionField = $referenceVersionField;
                        break;
                    }
                }
            ...
            if ($field->is(SetNullOnDelete::class)) {
                $delete = 'SET NULL';
            } elseif ($field->is(RestrictDelete::class)) {
                $delete = 'RESTRICT';
            } elseif ($field->is(CascadeDelete::class)) {
                $delete = 'CASCADE';
            } elseif ($definition instanceof EntityTranslationDefinition) {
                $delete = 'CASCADE'; // in der ManyToOneAssociationField von EntityTranslationDefinition fehlt CascadeDelete
            } elseif ($definition instanceof MappingEntityDefinition) {
                $delete = 'CASCADE';
            } elseif ($fkField->is(PrimaryKey::class) || $fkField->is(Required::class)) {
                $delete = 'CASCADE';
            } else {
                $delete = 'SET NULL';
            }
