...
export default class MoorlProductBuyListPlugin extends Plugin {
    ...
    _registerEvents() {
        ...
        this.el.addEventListener('change', (event) => {
            ...
            if (event.target.nodeName === 'SELECT') {
                ...
                that._client.get(
                    actionUrl + '?' + new URLSearchParams(query).toString(),
                    (response) => {
    ...
    _createFormValues(productId, quantity) {
        this._formValuesElement.appendChild(
            this._createFormValue(`lineItems[${productId}][id]`, productId)
        );
        this._formValuesElement.appendChild(
            this._createFormValue(`lineItems[${productId}][type]`, 'product')
        );
        this._formValuesElement.appendChild(
            this._createFormValue(
                `lineItems[${productId}][referencedId]`,
                productId
            )
        );
        this._formValuesElement.appendChild(
            this._createFormValue(`lineItems[${productId}][stackable]`, 1)
        );
        this._formValuesElement.appendChild(
            this._createFormValue(`lineItems[${productId}][removable]`, 1)
        );
        this._formValuesElement.appendChild(
            this._createFormValue(`lineItems[${productId}][quantity]`, quantity)
        );
    }
